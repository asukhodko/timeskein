...
use crate::domain::{DenylistPolicy, DenylistRule, DenylistRuleView};
use crate::AppState;
...
pub async fn handle_settings_set(
    ...
    if let Some(obj) = params.as_object() {
        for (key, value) in obj {
            ...
            state.db.set_setting(key, &value_str).await.map_err(|e| {
                RpcResponse::error(request_id.to_string(), "internal_error", &e.to_string())
            })?;
        }
    }
...
pub async fn handle_settings_get_denylist(
    ...
    let rules = state.db.get_denylist().await.unwrap_or_default();
...
pub async fn handle_settings_add_to_denylist(
    ...
    let policy = DenylistPolicy::from_str(policy_str).ok_or_else(|| {
        RpcResponse::error(request_id.to_string(), "validation_error", "Invalid policy")
    })?;
    ...
    state.db.add_denylist_rule(&rule).await.map_err(|e| {
        RpcResponse::error(request_id.to_string(), "internal_error", &e.to_string())
    })?;
...
pub async fn handle_settings_remove_from_denylist(
    ...
    let removed = state.db.remove_denylist_rule(id).await.map_err(|e| {
        RpcResponse::error(request_id.to_string(), "internal_error", &e.to_string())
    })?;
