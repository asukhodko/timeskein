...
use crate::domain::{
    FocusSession, WorkItem, WorkItemEvent, WorkItemEventKind, WorkItemState, WorkItemType,
};
use crate::AppState;
...
pub async fn handle_work_item_create(
    ...
    if should_start_focus {
        ...
        focus_session_id = activate_work_item_for_focus(&state, &mut item, request_id).await?;
