...
use crate::domain::{
    Ref, RefConflict, RefKind, RefView, WorkItemEvent, WorkItemEventKind, WorkItemRef,
};
...
impl Database {
    ...
    pub async fn check_ref_conflict(
        &self,
        kind: RefKind,
        normalized_value: &str,
    ) -> Result<Option<RefConflict>> {
        let row = sqlx::query(
    ...
    pub async fn remove_ref(&self, work_item_id: Uuid, ref_id: Uuid) -> Result<bool> {
        // Remove junction
        let result =
            sqlx::query("DELETE FROM work_item_refs WHERE work_item_id = ?1 AND ref_id = ?2")
                .bind(work_item_id.to_string())
        ...
        let count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM work_item_refs WHERE ref_id = ?1")
                .bind(ref_id.to_string())
                .fetch_one(self.pool())
                .await?;
    ...
    pub async fn has_refs(&self, work_item_id: Uuid) -> Result<bool> {
        let count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM work_item_refs WHERE work_item_id = ?1")
                .bind(work_item_id.to_string())
                .fetch_one(self.pool())
                .await?;
