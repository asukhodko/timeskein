...
- `pnpm dogfood:finish` checks that the day can be closed with no active focus session or active Work Item, then prints the Markdown day-closure report; when closing is blocked, its Markdown diagnostic uses Russian headings, blocker explanations, 24-hour time, and next steps
