...
* `docs/rfc/0002-system-topology-and-component-map.md` (топология и карта компонентов)
* `docs/mvp/02_user_story_manual_inventory.md` (manual-first inventory)
* `docs/mvp/02_manual_inventory_ui_ux.md` (UI/UX для manual-first inventory)
* `docs/adr/0001-initial-architecture.md` (первичная архитектура MVP)
...
### Уровень 0: Manual-first inventory (user-story-02)
...
### Уровень 2: Lightweight context (semantics-first)

* добавляются connectors (браузер, IDE, заметки, мессенджеры) как источники событий
* всё ещё без “тотального наблюдения”

### Уровень 3: Full context collection

* добавляются collectors per platform, включаемые пользователем
