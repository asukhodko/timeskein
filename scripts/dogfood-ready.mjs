...
if (!existsSync(appBundlePath)) {
  warnings.push("macOS-приложение ещё не собрано. Сначала выполни `pnpm dogfood:start:verified`, чтобы собрать и полностью проверить его.");
}
...
function appendDailyControlChecklist(lines) {
  ...
  lines.push("- Причинная цепочка в `Рабочей реальности`: кнопка `Начать отсюда` сама сохраняет намерение. После работы добавь к делу `Результат` с Ref, затем зафиксируй `Следующий шаг`; отдельную запись «намерение» создавать не нужно.");
