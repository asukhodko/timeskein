...
function appendDailyControlChecklist(lines) {
  ...
  lines.push("- Если хочется спокойно проверить готовность до строгого gate, используй `pnpm dogfood:goal-check:status -- --date YYYY-MM-DD`: он не закрывает цель и не запускает дорогие проверки.");
