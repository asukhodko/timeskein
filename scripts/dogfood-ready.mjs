...
function appendDailyControlChecklist(lines) {
  ...
  lines.push("- День считается доказательством цели, когда `pnpm dogfood:finish:save` печатает `Короткое закрытие: Закрытие уложилось в 10 минут: да (...)` и точную команду `pnpm dogfood:goal-check -- --date YYYY-MM-DD`.");
