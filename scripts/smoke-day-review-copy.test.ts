...
test('gap review helpers keep repeated Explain actions on the next unresolved gap', () => {
  ...
  const shiftedEnd = new Date(new Date(gaps[0].to).getTime() + 60_000).toISOString()
  const shiftedOpenGapText = formatGapDayEventDraft(gaps[0], 'Открытый разрыв', 'recovery')
    .replace(formatClockTime(gaps[0].to), formatClockTime(shiftedEnd))
  assert.equal(countClassifiedGaps(gaps, [{ text: shiftedOpenGapText, activity_zone: 'recovery' }]), 1)
